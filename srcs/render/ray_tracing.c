...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/14 11:20:24 by wdwain            #+#    #+#             */
/*   Updated: 2022/10/14 11:20:26 by wdwain           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ray_tracing_logic(t_ray_tracing rt, t_filtered_elements elements)
{
	...
	while (rt.x_angle <= WINDOW_WIDTH / 2)
	{
		...
		ray_direction_from_camera = camera_direction_calc(*rt.camera, \
															rt.mlx_x, rt.mlx_y);
		vec_norm(&ray_direction_from_camera);
		ray = ray_new(rt.camera->origin, &ray_direction_from_camera);
...
void	ray_tracing(t_mlx *mlx, t_filtered_elements elements)
{
	...
	rt.vplane = get_view_plane(\
		WINDOW_WIDTH, WINDOW_HEIGHT, rt.camera->fov);
	...
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_win_ptr, rt.image.img, 0, 0);
